...
| Task | `lib/task-ledger.js`, `lib/task-orchestration/task-orchestrator.js`, `lib/task-registry.js` | `.ephemeral/task-ledger.json`, `.ephemeral/task-runs.json`, plugin task records, cron records | `server/routes/tasks.js`, task tools, channel task tool, subagent tool, cron scheduler, board UI | `/api/tasks`, `/api/tasks/runs`, `TaskPage`, task graph store | task ledger updates, task graph progress events, app events | `tests/task-ledger.test.js`, `tests/task-orchestrator.test.js`, `tests/task-registry.test.js`, `tests/tasks-route.test.js`, `tests/channel-router-reply-tools.test.js` | Task Ledger is the durable source, but orchestration, plugin tasks, channel follow-up tasks, cron and board UI each still own execution details. |
| Channel | `lib/channels/`, `hub/channel-router.js` | Channel Markdown files, per-agent channel membership/read position, optional project snapshot metadata | `server/routes/channels.js`, channel tools, channel router | `/api/channels`, channel page, channel store | channel WebSocket messages, channel activity events | `tests/channels-route.test.js`, `lib/channels/TDD.md` mapped tests | Channel file contract is clear; bridge group protocol is intentionally outside this module; linked projects are snapshots, not dynamic routing authority. |
| Project | `lib/projects/project-registry.js`, `server/routes/projects.js` | `AGENTRY_HOME/projects.json` under the user data directory | `/api/projects`, settings Projects tab, channel creation project selector | `/api/projects`, settings project list, channel project snapshot fields | `projects-changed` app event | `tests/projects-route.test.js`, `tests/channels-route.test.js`, `lib/projects/TDD.md` | Project registry owns local metadata and validation; workspace approval, task board execution, and channel membership remain separate domains. |
| Computer Use | `core/computer-use/`, `lib/tools/computer-use-tool.js` | Computer Use settings, app approval records, in-memory session leases and snapshots | `computer` tool, `server/routes/preferences.js`, provider helper commands | `ComputerHost.getStatus()`, provider status/snapshots, Computer Use settings tab | `computer_overlay` events, input-area app approval confirmations | `tests/computer-use-*.test.js`, `desktop/src/react/settings/tabs/__tests__/ComputerUseTab.test.tsx`, `core/computer-use/TDD.md` | Provider capability gates and OS permission surfaces are safety boundaries; do not route GUI control around ComputerHost. |
...
| Channel file contract | `lib/channels/EARS.md`, `lib/channels/BDD.md`, `lib/channels/TDD.md` | Channel Markdown persistence, member validation, and linked project snapshots stay under `lib/channels/`. |
| Project registry contract | `lib/projects/EARS.md`, `lib/projects/BDD.md`, `lib/projects/TDD.md` | Project metadata persistence and root validation stay under `lib/projects/`; consumers store snapshots or references explicitly. |
| Computer Use host contract | `core/computer-use/EARS.md`, `core/computer-use/BDD.md`, `core/computer-use/TDD.md` | Provider selection, app leases, snapshots, capability checks, app approvals and OS helper normalization stay behind `ComputerHost` and provider contracts. |
