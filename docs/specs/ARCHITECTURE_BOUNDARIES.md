...
| Workspace | `core/config-coordinator.js`, `shared/workspace-history.js`, `shared/workspace-scope.js` | Agent `config.yaml` fields such as `desk.home_folder`, `last_cwd`, `cwd_history`; workspace UI preferences | `/api/agents/:id/config`, `/api/config/workspaces/recent`, session creation/switch paths | `engine.getHomeCwd()`, `/api/config`, `/api/agents/switch`, desktop workspace store | `agent-workspace-changed`, `agent-switched`, workspace UI persistence | `tests/workspace-scope.test.js`, `tests/config-workspaces-route.test.js`, desktop desk-action tests | Workspace defaults, explicit home folder and session cwd still need a sharper source-of-truth rule. |
