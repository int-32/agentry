...
function normalizeChannelProject(project) {
  ...
  return {
    ...
    workspaceRoot: typeof project.workspaceRoot === "string" ? project.workspaceRoot.trim() : "",
    docsRoot: typeof project.docsRoot === "string" ? project.docsRoot.trim() : "",
    testCommand: typeof project.testCommand === "string" ? project.testCommand.trim() : "",
    description: typeof project.description === "string" ? project.description.trim() : "",
  };
}

function normalizeChannelTaskBoard(taskBoard) {
  if (!taskBoard || typeof taskBoard !== "object") return null;
  const id = typeof taskBoard.id === "string"
    ...
    : typeof taskBoard.boardId === "string"
      ...
      : typeof taskBoard.taskBoardId === "string"
        ...
        : "";
  if (!id) return null;
  const selectedAgentIds = Array.isArray(taskBoard.selectedAgentIds)
    ? Array.from(new Set(taskBoard.selectedAgentIds.map((item) => String(item || "").trim()).filter(Boolean)))
    : [];
  const coordinatorAgentId = typeof taskBoard.coordinatorAgentId === "string"
    ? taskBoard.coordinatorAgentId.trim()
    : "";
  return {
    id,
 ...
 * @param {object} [opts.project] - 关联项目快照
 * @param {object} [opts.taskBoard] - 关联看板执行域
 * @param {string} [opts.intro] - 频道介绍（作为第一条系统消息）
 * @returns {{ filePath: string, id: string }}
 */
export async function createChannel(channelsDir, { id, name, description, members, project, taskBoard, intro }) {
  ...
  return withFileLock(filePath, async () => {
    ...
    const normalizedProject = normalizeChannelProject(project);
    if (normalizedProject) {
      meta.projectId = normalizedProject.id;
      meta.projectName = normalizedProject.name;
      meta.projectWorkspaceRoot = normalizedProject.workspaceRoot;
      meta.projectDocsRoot = normalizedProject.docsRoot;
      meta.projectTestCommand = normalizedProject.testCommand;
      meta.projectDescription = normalizedProject.description;
    }
    const normalizedTaskBoard = normalizeChannelTaskBoard(taskBoard);
    if (normalizedTaskBoard) {
      meta.taskBoardId = normalizedTaskBoard.id;
      meta.taskBoardTitle = normalizedTaskBoard.title;
      meta.taskBoardCoordinatorAgentId = normalizedTaskBoard.coordinatorAgentId;
      meta.taskBoardSelectedAgentIds = normalizedTaskBoard.selectedAgentIds;
    }
