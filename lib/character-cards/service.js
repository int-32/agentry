...
export function createCharacterCardService(engine) {
  const stagingRoot = path.join(engine.agentryHome, ".ephemeral", "character-card-imports");
  ...
  async function exportAgentPackage(agentId, options = {}) {
    ...
    const token = randomToken();
    const exportRoot = path.join(engine.agentryHome, ".ephemeral", "character-card-exports", token);
