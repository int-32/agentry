 ...
 * @param {string} opts.agentryHome
 * @param {"standard"|"full-access"} opts.mode
 * @returns {object} policy
 */
export function deriveSandboxPolicy({ agentDir, workspace, workspaceFolders = [], agentryHome, mode }) {
  if (mode === "full-access") {
  ...
  return {
    ...
    writablePaths: [
      ...
      ...READ_WRITE_HOME_DIRS.map((d) => path.join(agentryHome, d)),
      ...workspaceRoots,
    ].filter(Boolean),

    // Windows AppContainer 用：显式只读路径。macOS/Linux 仍保留全局读取 +
    // denyReadPaths 的现有语义，Windows 需要把可读资源投影成 ACL grant。
    readablePaths: [
      ...READ_ONLY_AGENT_FILES.map((f) => path.join(agentDir, f)),
      ...READ_ONLY_AGENT_DIRS.map((d) => path.join(agentDir, d)),
      ...READ_ONLY_HOME_DIRS.map((d) => path.join(agentryHome, d)),
    ].filter(Boolean),

    // OS 沙盒用：拒绝读取（文件 + 目录）
    denyReadPaths: [
      ...BLOCKED_FILES.map((f) => path.join(agentryHome, f)),
      ...BLOCKED_DIRS.map((d) => path.join(agentryHome, d)),
    ],
