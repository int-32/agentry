...
export class PathGuard {
  ...
  constructor(policy) {
    ...
    this.agentryHome = this._resolveReal(policy.agentryHome) || path.resolve(policy.agentryHome);
  ...
  getAccessLevel(rawPath) {
    ...
    // 1. BLOCKED 文件（agentryHome 根）
    for (const f of BLOCKED_FILES) {
      if (resolved === path.join(this.agentryHome, f)) return AccessLevel.BLOCKED;
    }

    // 2. BLOCKED 目录
    for (const d of BLOCKED_DIRS) {
      if (this._isInside(resolved, path.join(this.agentryHome, d))) {
    ...
    // 8. agentryHome 内未匹配 → 安全兜底 BLOCKED
    if (this._isInside(resolved, this.agentryHome)) return AccessLevel.BLOCKED;
