...
async function persistArtifactFile({
  ...
  const agentryHome = getHanakoHome?.() || null;
  if (!agentryHome) throw new Error("create_artifact requires agentryHome to persist generated artifacts");
  ...
  const dir = sessionFilesCacheDir(agentryHome, sessionPath);
