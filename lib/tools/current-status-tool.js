...
export function createCurrentStatusTool(deps = {}) {
  ...
  return {
    ...
    description:
      "Lightweight current-environment status. Use it to check fresh state such as time, logical date, agent identity, model identity, Bridge platform context, and the user's visible UI context. IMPORTANT: The date/time shown in the system prompt is a snapshot from when the session was created and may be outdated. For precise current time, scheduling, reminders, date calculations that need clock time, greetings, or any case where hour/minute matters, call this tool with action=\"get\", key=\"time\". Use key=\"logical_date\" only when you need to know which calendar day Agentry should treat as today under the 4:00 boundary; it does not return hour/minute/second. When the user refers to this file, here, the current/open/visible/selected item, the pinned window, current Bridge chat/platform, or the current folder, call this tool first to resolve the reference.",
    parameters: Type.Object({
      action: StringEnum(["list", "get"], {
        description: "list returns available status keys; get returns one status key value.",
      }),
      key: Type.Optional(Type.String({
        description: "Status key to fetch when action=get. Common keys: time, logical_date, agent, model, ui_context, session_files, bridge_context. Use bridge_context for references to the current Bridge platform/chat.",
      })),
    }),
