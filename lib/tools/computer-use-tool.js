...
function formatElementSummary(elements = [], { limit = 60 } = {}) {
  const rows = (Array.isArray(elements) ? elements : [])
    .map((element) => ({
      ...
      bounds: formatBounds(element.bounds),
      confidence: formatNumber(element.confidence),
    }))
  ...
  const lines = rows.map((element) => {
    ...
    const metadata = [
      element.bounds,
      element.confidence ? `confidence=${element.confidence}` : null,
    ].filter(Boolean);
    const suffix = metadata.length ? ` (${metadata.join(", ")})` : "";
    return `- ${element.elementId}: ${element.role}${label}${disabled}${suffix}`;
  });
...
function formatRecognitionSummary(snapshot = {}) {
  const recognition = snapshot.recognition;
  if (!recognition || typeof recognition !== "object") return null;
  const source = recognition.primarySource || "unknown";
  const coverage = formatNumber(recognition.coverageScore);
  const averageConfidence = formatNumber(recognition.averageConfidence);
  const parts = [
    coverage ? `coverage=${coverage}` : null,
    averageConfidence ? `avgConfidence=${averageConfidence}` : null,
    Number.isFinite(Number(recognition.elementCount)) ? `elements=${recognition.elementCount}` : null,
    Number.isFinite(Number(recognition.actionableCount)) ? `actionable=${recognition.actionableCount}` : null,
    Number.isFinite(Number(recognition.labeledCount)) ? `labeled=${recognition.labeledCount}` : null,
  ].filter(Boolean);
  const lines = [`- recognition: ${source}${parts.length ? ` ${parts.join(", ")}` : ""}`];
  if (recognition.visualFallbackRecommended === true) {
    lines.push("- AX coverage is weak; compare with the screenshot before choosing an element id and report uncertainty when the target is not exposed.");
  }
  return lines.join("\n");
}
...
function buildAppStateText(snapshot) {
  const recognitionSummary = formatRecognitionSummary(snapshot);
  return [
...
export function createComputerUseTool(options = {}) {
  return {
    name: "computer",
    label: "Computer Use",
    description: "Inspect and control an approved desktop app or window through Agentry Computer Use.",
      ...
      try {
        ...
        switch (params.action) {
          ...
          case "get_app_state": {
            ...
            return {
              ...
              details: {
                ...
                recognition: snapshot.recognition || null,
              },
            };
          }
          case "click_element":
          ...
          case "perform_secondary_action": {
            const leaseParams = resolveCurrentLeaseParams(host, toolCtx, params);
            const presentation = host.getActionPresentation?.(toolCtx, leaseParams.leaseId, params.action, params) || {};
