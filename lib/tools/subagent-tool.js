...
export function createSubagentTool(deps) {
  ...
  return {
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      const ledgerTask = ledger?.createTask?.({
        title: taskSummary || "Subagent task",
        body: params.task,
        status: "running",
        source: { type: "subagent", subagentTaskId: taskId },
        assignee: targetAgentId ? { type: "agent", id: targetAgentId } : { type: "agent", id: deps.currentAgentId || "self" },
        rootSessionPath: parentSessionPath,
        cwd: parentCwd || null,
        contextRefs: [{ type: "deferred", taskId }],
      });
      ...
      registry?.register(taskId, {
        ...
        meta: applyRequestedAgentMetadata(
          mergeExecutorMetadata({ summary: taskSummary, taskLedgerTaskId: ledgerTask?.id || null }, requestedIdentity),
          requestedIdentity,
        ),
      });
        ...
        return deps.executeIsolated(
          params.task,
          {
            ...
            onSessionReady: (sp) => {
              ...
              store._save?.();
              if (ledgerTask?.id) {
                ledger?.addComment?.(ledgerTask.id, {
                  author: "system",
                  body: `Subagent session ready: ${sp}`,
                  channel: "subagent",
                  meta: { sessionPath: sp, taskId },
                });
              }
      ...
      executeForAgent(targetAgentId).then(result => {
        ...
        if (wasUserAborted) {
          ...
          if (ledgerTask?.id) {
            ledger?.updateTask?.(ledgerTask.id, { status: "cancelled" });
            ledger?.addComment?.(ledgerTask.id, {
              author: "subagent",
              body: t("error.subagentAborted"),
              channel: "subagent",
              meta: { taskId, aborted: true },
            });
          }
          deps.emitEvent?.({
            type: "block_update", taskId,
        ...
        if (ledgerTask?.id) {
          ledger?.updateTask?.(ledgerTask.id, { status: outcome.ok ? "done" : "failed" });
          ledger?.addComment?.(ledgerTask.id, {
            author: "subagent",
            body: (summary || "").slice(0, 4000),
            channel: "subagent",
            meta: { taskId, ok: outcome.ok },
          });
        }
        deps.emitEvent?.({
          type: "block_update", taskId,
        ...
        if (ledgerTask?.id) {
          ledger?.updateTask?.(ledgerTask.id, { status: wasUserAborted ? "cancelled" : "failed" });
          ledger?.addComment?.(ledgerTask.id, {
            author: "subagent",
            body: reason,
            channel: "subagent",
            meta: { taskId, aborted: wasUserAborted },
          });
        }
      ...
      return {
        ...
        details: {
          taskId,
          ledgerTaskId: ledgerTask?.id || null,
          task: params.task,
          taskTitle,
