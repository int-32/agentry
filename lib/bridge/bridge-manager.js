...
export class BridgeManager {
  ...
  constructor({ engine, hub }) {
    ...
    this.mediaPublisher = new MediaPublisher({
      baseUrl: engine.getBridgeMediaPublicBaseUrl?.()
        || process.env.AGENTRY_BRIDGE_PUBLIC_BASE_URL
        || process.env.HANA_BRIDGE_PUBLIC_BASE_URL
        || "",
      allowedRoots: roots,
    });
  ...
  autoStart(agents) {
    ...
    for (const [agentId, agent] of agents) {
      ...
      for (const [platform, spec] of Object.entries(ADAPTER_REGISTRY)) {
        ...
        if (platform === "wechat") cfg._hanaHome = this.engine.agentryHome;
  ...
  _sendLlmWaitingReceipt(platform, chatId, agentId, replyContext = null) {
    ...
    if (agentName && adapter.sendReply) {
      this._sendAdapterReply(adapter, chatId, `（${agentName}正在输入...）`, replyContext).catch(() => {});
  ...
  async _handleMessage(platform, msg) {
    ...
    const configuredOwner = this._isOwner(platform, userId, agentId, { isGroup });
    // Social/bridge messages are treated as full local Agent sessions. Keep
    // configuredOwner only for sender labeling; execution and bridge commands
    // intentionally run with owner-level access.
    const isOwner = true;

    // ── Slash 命令拦截（统一走 dispatcher）──
    // 纪律：
    //   1) 斜杠命令必须始终优先（任何 pending/attached 状态都不能夺走斜杠通道）
    //   2) 社交入口现在统一按本地完整 Agent 执行；斜杠命令也按 owner 级别处理。
    ...
    // pending-selection 按 sessionKey keying。
    ...
    // ── 私聊：debounce + abort ──
    const line = !configuredOwner && senderName
  ...
  _createStreamDelivery({ adapter, chatId, isGroup, messageThreadId, replyContext }) {
  ...
  async _flushPending(sessionKey) {
    ...
    try {
      ...
      // 社交入口统一 full access 后，/rc attachment 也按完整 Agent 权限处理。
      const rcState = this.engine.rcState;
