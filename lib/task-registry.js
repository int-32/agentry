...
export class TaskRegistry {
  ...
  register(taskId, { type, parentSessionPath = null, meta = {}, pluginId = null, agentId = null, persist = true } = {}) {
    ...
    const incomingMeta = objectOrEmpty(meta);
    const now = Date.now();
    const task = {
      taskId: id,
      type: taskType,
      ...
      ledgerTaskId: existing?.ledgerTaskId || incomingMeta.taskLedgerTaskId || null,
    };
    task.meta = { ...task.meta, ...incomingMeta };
  ...
  _mirrorToLedger(task) {
    if (!this._taskLedger?.mirrorRegistryTask) return;
    try {
      const mirrored = this._taskLedger.mirrorRegistryTask(task);
      if (mirrored?.id && task && !task.ledgerTaskId) {
        task.ledgerTaskId = mirrored.id;
        task.meta = { ...objectOrEmpty(task.meta), taskLedgerTaskId: mirrored.id };
      }
    } catch (err) {
      console.warn(`[task-registry] ledger mirror failed for ${task?.taskId}: ${err.message}`);
    }
  }
