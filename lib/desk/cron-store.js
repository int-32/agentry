...
export class CronStore {
  ...
  constructor(jobsPath, runsDir, options = {}) {
    ...
    this._taskLedger = options.taskLedger || null;
    this._agentId = typeof options.agentId === "string" ? options.agentId : null;
    this._jobs = [];
    this._nextNum = 1;
    this._load();
    let mirroredAny = false;
    for (const job of this._jobs) {
      const before = job.taskLedgerId;
      this._mirrorJobToLedger(job);
      if (!before && job.taskLedgerId) mirroredAny = true;
    }
    if (mirroredAny) this._save();
  }

  setTaskLedger(taskLedger, { agentId = this._agentId } = {}) {
    this._taskLedger = taskLedger || null;
    this._agentId = typeof agentId === "string" ? agentId : this._agentId;
  }
  ...
  removeJob(id) {
    ...
    this._mirrorJobToLedger({ ...this._jobs[idx], enabled: false }, { state: "archived" });
  ...
  logRun(jobId, run) {
    ...
    if (job) this._recordRunToLedger(job, run);
  ...
  _mirrorJobToLedger(job, opts = {}) {
    if (!this._taskLedger?.mirrorCronJob || !job) return;
    try {
      const mirrored = this._taskLedger.mirrorCronJob(job, { agentId: this._agentId, ...opts });
      if (mirrored?.id && !job.taskLedgerId) job.taskLedgerId = mirrored.id;
    } catch (err) {
      console.warn(`[CronStore] TaskLedger mirror failed for ${job?.id}: ${err.message}`);
    }
  }

  _recordRunToLedger(job, run) {
    if (!this._taskLedger?.recordCronRun || !job) return;
    try {
      const mirrored = this._taskLedger.recordCronRun(job, run, { agentId: this._agentId });
      if (mirrored?.id && !job.taskLedgerId) job.taskLedgerId = mirrored.id;
    } catch (err) {
      console.warn(`[CronStore] TaskLedger cron run mirror failed for ${job?.id}: ${err.message}`);
    }
  }
