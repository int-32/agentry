...
export const DEFAULT_OFFICIAL_PLUGIN_MARKETPLACE_URL = "https://raw.githubusercontent.com/int-32/agentry-plugins/main/marketplace.json";
...
export function createDefaultPluginMarketplace({ agentryHome, env = process.env, fetchImpl } = {}) {
  const envPath = normalizeOptionalText(env.AGENTRY_PLUGIN_MARKETPLACE_FILE || env.HANA_PLUGIN_MARKETPLACE_FILE);
  const envUrl = normalizeOptionalText(env.AGENTRY_PLUGIN_MARKETPLACE_URL || env.HANA_PLUGIN_MARKETPLACE_URL);
  const localIndex = agentryHome ? path.join(agentryHome, "plugin-marketplace", "marketplace.json") : null;
