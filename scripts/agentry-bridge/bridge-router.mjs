...
import { antigravityWorkspaceUris } from "./bridge-workspace.mjs";
...
async function streamClaude({ res, id, model, prompt, systemPrompt, conversationFp, workspaceConfig = {}, sessionStore }) {
  // Claude SDK 隔离配置：
  ...
  const resumeId = conversationFp && sessionStore ? sessionStore.lookupSession(conversationFp, "claude") : null;
  ...
  if (conversationFp && capturedSessionId && sessionStore) {
    sessionStore.rememberSession(conversationFp, "claude", capturedSessionId);
  }
...
async function streamCodex({ res, id, model, prompt, systemPrompt, conversationFp, workspaceConfig = {}, sessionStore }) {
  ...
  const resumeId = conversationFp && sessionStore ? sessionStore.lookupSession(conversationFp, "codex") : null;
  ...
  if (conversationFp && capturedThreadId && sessionStore) {
    sessionStore.rememberSession(conversationFp, "codex", capturedThreadId);
  }
  writeDone(res, id, model, "stop", usage && {
    prompt_tokens: usage.input_tokens || 0,
    completion_tokens: usage.output_tokens || 0,
...
export const BRIDGE_MODEL_CATALOG = [
...
export function createBridgeRequestHandler({ cliProviderConfigLoader } = {}) {
  const providerConfigLoader = cliProviderConfigLoader || createCliProviderConfigLoader({
    agentryHome: resolveBridgeAgentryHome(process.env),
  });
  const sessionStore = createConversationSessionStore();
  ...
  return async function handleBridgeRequest(req, res) {
    ...
    if (req.method === "POST" && (req.url === "/v1/chat/completions" || req.url === "/chat/completions")) {
      ...
      req.on("end", async () => {
        ...
        const { backend, workspaceConfig } = resolveBridgeRuntimeConfig(model, providerConfigLoader);
        ...
        if (!wantStream) {
          ...
          try {
            if (backend === "claude") await streamClaude({ res: fakeRes, id, model, prompt, systemPrompt, conversationFp, workspaceConfig, sessionStore });
            else if (backend === "codex") await streamCodex({ res: fakeRes, id, model, prompt, systemPrompt, conversationFp, workspaceConfig, sessionStore });
        ...
        try {
          if (backend === "claude") await streamClaude({ res, id, model, prompt, systemPrompt, conversationFp, workspaceConfig, sessionStore });
          else if (backend === "codex") await streamCodex({ res, id, model, prompt, systemPrompt, conversationFp, workspaceConfig, sessionStore });
